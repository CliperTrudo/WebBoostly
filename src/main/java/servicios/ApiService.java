...
/**
 * Clase ApiService que se encarga de gestionar las solicitudes HTTP hacia la
 * API REST. Realiza acciones como obtener, actualizar y eliminar proyectos, así
 * como registrar usuarios y gestionar la autenticación.
 */
public class ApiService {
	...
	// Constructor que inicializa el ObjectMapper con los módulos necesarios para el
	// manejo de fechas
	public ApiService() {
	 ...
	 * @param jsonInput El cuerpo de la solicitud, en formato JSON, o null si no se
	 *                  necesita.
	 * @return La respuesta de la API en formato JSON, o null si la solicitud no fue
	 *         exitosa.
	 ...
	 * @return true si el proyecto fue eliminado correctamente, false si ocurrió un
	 *         error.
	 */
	...
	/**
     * Obtiene un usuario por su ID (GET /api/usuarios/{id}).
     */
    public UsuarioDto obtenerUsuarioPorId(Long idUsuario) {
        try {
            ...
            return response != null 
                ? mapper.readValue(response, UsuarioDto.class) 
                : null;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    /**
     * Actualiza un usuario (PUT /api/usuarios/actualizar/{id}).
     * @param usuarioDto Objeto usuario ya con ID y campos a modificar.
     * @return true si la API devolvió éxito, false si no.
     */
    public boolean actualizarUsuario(UsuarioDto usuarioDto) {
        try {
            // Serializo el DTO a JSON
            String jsonInput = mapper.writeValueAsString(usuarioDto);

            // Hago PUT a /usuarios/actualizar/{id}
            String response = sendHttpRequest(
                API_BASE_URL + "/usuarios/actualizar/" + usuarioDto.getId(),
                "PUT",
                jsonInput
            );
            // La API, si todo va bien, suele devolver el objeto JSON actualizado.
            // Si 'response' no es null, consideramos que fue exito.
            return response != null;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    /**
     * Elimina un usuario dado su ID invocando DELETE /api/usuarios/{id}.
     * @param idUsuario El ID del usuario a eliminar.
     * @return true si la eliminación fue exitosa (código HTTP 200 o 204); false en otro caso.
     */
    public boolean eliminarUsuario(Long idUsuario) {
        try {
            // Hacemos DELETE a /usuarios/{id}
            return sendHttpRequest(API_BASE_URL + "/usuarios/" + idUsuario, "DELETE", null) != null;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
	 ...
	 * @return true si el proyecto fue actualizado correctamente, false si ocurrió
	 *         un error.
	 */
	 ...
	 * 
	 * @param idUsuario El ID del usuario para el que se deben obtener los
	 *                  proyectos.
	 * @return Una lista de proyectos del usuario, o null si no se encuentran
	 *         proyectos.
	 */
	 ...
	 * 
	 * @param usuDto El DTO que contiene los datos del usuario que intenta iniciar
	 *               sesión.
	 * @return La respuesta de la API en formato String.
	 */
	...
	public List<ProyectoDto> obtenerProyectos() {
		try {
			...
			String response = sendHttpRequest(API_BASE_URL + "/proyectos", "GET", null);
			return response != null ? Arrays.asList(mapper.readValue(response, ProyectoDto[].class)) : null;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
