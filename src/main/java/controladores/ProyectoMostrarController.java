 ...
 * convierte cada imagen a Base64 y las asigna a atributos de request.
 * Además, invoca DonacionService para recuperar todas las donaciones
 * pendientes de autorización (estado = "PENDIENTE_AUTORIZACION") para ese proyecto.
 *
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (proyecto.getImagen1Proyecto() != null) {
                String imagen1Base64 = Base64.getEncoder()
                        .encodeToString(proyecto.getImagen1Proyecto());
                request.setAttribute("imagen1Base64", imagen1Base64);
            }
            if (proyecto.getImagen2Proyecto() != null) {
                String imagen2Base64 = Base64.getEncoder()
                        .encodeToString(proyecto.getImagen2Proyecto());
                request.setAttribute("imagen2Base64", imagen2Base64);
            }
            if (proyecto.getImagen3Proyecto() != null) {
                String imagen3Base64 = Base64.getEncoder()
                        .encodeToString(proyecto.getImagen3Proyecto());
                request.setAttribute("imagen3Base64", imagen3Base64);
            }

            // 5) Recuperar todas las donaciones pendientes de autorización para este proyecto
            List<DonacionDto> donacionesPendientes = donacionService
                    .obtenerDonacionesPendientes(idProyecto);
