...
/**
 * Servlet que muestra el formulario de edición de un usuario y procesa
 * el envío de los datos modificados para actualizar el usuario vía API.
 */
...
public class UsuarioEditarController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        SesionDto sesionUsu = (session != null)
            ? (SesionDto) session.getAttribute("datos")
            : null;
        if (sesionUsu == null) {
        ...
        try {
            ...
            if (ok) {
                ...
                response.sendRedirect(request.getContextPath()
                    + "/editarUsuario?id=" + idUsuario + "&error=1");
            }
